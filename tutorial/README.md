...
인증보기와 템플릿은 작동하지만 지금은 매우 단순 해 보입니다. 구성한 HTML 레이아웃에 스타일을 추가하기 위해 일부 CSS를 추가 할 수 있습니다. 스타일은 변경되지 않으므로 템플릿이 아닌 정적 파일입니다.

Flask는 flaskr/static 디렉터리에 상대적인 경로를 가져와 제공하는 정적보기를 자동으로 추가합니다. base.html 템플릿에는 이미 style.css 파일에 대한 링크가 있습니다.

CSS 외에 다른 유형의 정적 파일은 JavaScript 기능이있는 파일 또는 로고 이미지 일 수 있습니다. 그것들은 모두 flaskr/static 디렉토리에 있으며 url_for('static', filename = '...')로 참조됩니다.

이 자습서는 CSS 작성 방법에 중점을 두지 않으므로 다음을 flaskr/static/style.css 파일에 복사하면 됩니다.

[Mozilla 문서](https://developer.mozilla.org/ko/docs/Web/CSS)에서 CSS에 대한 자세한 내용을 읽을 수 있습니다. 정적 파일을 변경하는 경우 브라우저 페이지를 새로 고칩니다. 변경 사항이 표시되지 않으면 브라우저의 캐시를 삭제 해보세요.